...
2048×1536, source at `C:/dev/Bug Me Not/bugmenot-html5`). It's wired up the
same way as Word Invader: `"play": { "dir": "bugmenot", "html5": true, ... }`
in `site.json`, wrapper generated by `build.mjs`. It also declares a
`play.font` (Varela Round) so the wrapper preloads the web font the game
rasterises text with. The `loader: { maxParallelDownloads: 4, maxRetries: 10 }`
block is in the game's own `src/main.ts`, so future builds keep it. The old
Unity build remains in `play/bug-me-not/` (unlinked; contains the stray
GoGammon copies — a delete candidate).
